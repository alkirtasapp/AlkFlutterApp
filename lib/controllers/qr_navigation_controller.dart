...
class QrNavigationController extends GetxController {
  ...
  Future<bool> _searchProductByBarcode(String barcode) async {
    try {
      ...
      // Check if product is active (handle various formats: 1, "1", true, "true")
      // NOTE: Allow viewing inactive products for in-store barcode scanning
      final active = product['active'];
      final isActive = active == 1 || active == '1' || active == true || active == 'true';
      // Product can be viewed and added to cart even if inactive on PrestaShop (for in-store use)
